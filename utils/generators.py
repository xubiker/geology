...
class AutoBalancedPatchGeneratorPolarized:
    ...
    def __init__(self, img_dir_path: Path, mask_dir_path: Path, cache_path: Path, add_img_dir_path: Path, valid_zones_dir_path: Path, patch_size: int, n_classes: int,
        ...
        self.valid_zones_paths = sorted(list(valid_zones_dir_path.iterdir()))
        ...
        self.valid_zones = self._load_valid_zones()
        self.masks = self._load_masks()
        self.prob_maps = self._load_maps(quiet=quiet)
        self.n_imgs = len(self.mask_paths)
        # --- calculate weights of images ---
    ...
    def _load_add_imgs(self):
        print('\t Loading additional images with polarized minerals...')
        add_imgs = []
        for add_path in tqdm(self.add_img_paths):
            add_imgs.append([np.array(Image.open(p)).astype(np.float32) / 255 for p in sorted(list(add_path.iterdir()))])
        add_imgs.append([np.zeros_like(add_imgs[0][0]) for p in sorted(list(self.add_img_paths[0].iterdir()))])
        return add_imgs

    def _load_valid_zones(self):
        print('\t Loading valid zones...')
        return [self._load_valid_zone(p) for p in tqdm(self.valid_zones_paths)]
    ...
    @staticmethod
    def _load_valid_zone(p: Path):
        mask = (np.array(Image.open(p)).astype(np.float32) / 255).astype(np.uint8)
        return mask if mask.ndim == 2 else mask[:, :, 0]
